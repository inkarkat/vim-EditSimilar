...
"				ingo#subst#tuples#Substitute() for reuse.
...
function! EditSimilar#Substitute#Open( opencmd, isCreateNew, filespec, ... )
	...
	let [l:replacementFilename, l:failedPatterns] = ingo#subst#tuples#Substitute(l:originalFilename, a:000)
	...
	if ! empty(l:failedPatterns)
	    ...
	    let [l:replacementPathspec, l:failedPatterns] = ingo#subst#tuples#Substitute(l:originalPathspec, l:failedPatterns)
		...
		let [l:replacementFilespec, l:failedPatterns] = ingo#subst#tuples#Substitute(l:replacementFilespec, filter(l:failedPatterns, 'ingo#regexp#fromwildcard#IsWildcardPathPattern(v:val)'))
	    endif
	endif
